...
class StaffMemberController extends GetxController {
  ...
  Future<void> fetchStaffMembers() async {
    try {
      ...
      final response = await http.get(Uri.parse("http://192.168.1.101:5274/api/staffmembers"));
  ...
  Future<void> fetchStaffMemberDetails(int id) async {
    try {
      isLoading.value = true; // Set loading to true
      final response = await http.get(Uri.parse("http://192.168.1.101:5274/api/staffmembers/$id"));
      if (response.statusCode == 200) {
  ...
  Future<void> deleteStaffMember(int id) async {
    try {
      ...
      final response = await http.delete(
        Uri.parse('http://192.168.1.101:5274/api/staffmembers/$id'),
      );
 ...
 Future<bool> createStaffMember(StaffMember newStaffMember) async {
  try {
    ...
    final response = await http.post(
      Uri.parse('http://192.168.1.101:5274/api/staffmembers'),
      headers: {"Content-Type": "application/json"},
